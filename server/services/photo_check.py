...
판정 기준 (전부 단일 출처 상수):
  - 마커 크기:  measure.MIN_MARKER_WIDTH_PX (작으면 척도 노이즈 증폭 — 2-7 실증)
...
from services.measure import MIN_MARKER_WIDTH_PX, TILT_RATIO_RANGE, compute_scale
...
def check_photo(image_bgr: np.ndarray, mode: str) -> dict:
    ...
    size_ok = trace["widthPx"] >= MIN_MARKER_WIDTH_PX
    if not size_ok:
        reasons.append("기준물이 너무 작게 보입니다 — 조금 더 가까이서 전신이 꽉 차게 찍어 주세요")
