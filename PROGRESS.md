...
- **다음 시작 지점: Phase 4 Gate 검증** — 자동 4항목(FitResult 산출·논리
  역전 0건·여유/부족 cm 표시·신뢰도 low 반영)은 pytest로 커버, 통합(촬영→
  측정→의류→핏)은 E2E 데스크톱판 통과. **수동 검증(사용자 몫, ★ 중요)**:
  본인 실측 치수 + **핏을 아는 옷 3개** 대조("이 브랜드 M이 맞는데 앱도 M
  추천하는가") + 자연어 피드백-수치 모순 확인 — 이것이 Phase 2 편향 일정성
  가정의 실질 검증. 어긋나면 EASE_RANGES·가중치 교정 또는 Phase 2 수동
  검증(보류분) 수행. 통과 시 qa Gate 리포트 → 사용자 승인 → Phase 5
