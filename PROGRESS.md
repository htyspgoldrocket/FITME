...
- **다음 시작 지점: Step 2-7c — 촬영 가이드 백엔드 (/check-photo + 대칭성 검사)**
  - 촬영 가이드 4층위가 제품 핵심 기능으로 격상됨 (CLAUDE.md 전략 1 하위 명세,
    배운 것 27번). UI 구현은 2-8, 백엔드(층위 3·4 판정)를 2-7c에서 먼저
  - 이후 2-7b(키 캘리브레이션·BMI — 계수 역산은 밀착 옷 데이터 후) → 2-8(4층위 UI)
