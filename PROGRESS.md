...
- **다음 시작 지점: 사용자 결정 대기** — ① 옵션 B 전환 여부(허리 일관성을
  알려진 한계로 문서화 — 신뢰도 시스템은 이미 low 강등으로 대응 중)
  ② 허리 프롬프트 정의를 신(기하 중간점)/구(natural waist) 중 무엇으로
  확정할지 ③ 확정 후 허리 깊이 계수 재역산(신정의면 0.777로 정상화)
