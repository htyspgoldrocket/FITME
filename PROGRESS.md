...
- **5-4 실기기 재검증 보류 (2026-07-21 사용자 결정) — 개선 백로그 ③~⑥ 먼저
  ...
  - **B-2 (④) ✅ 완료 (2026-07-21)**: 실루엣 거리 자 — 실루엣 SVG 내부에
    ...
    **구현 교훈**: 처음 HTML 퍼센트 역산(5.8%/82.2%)으로 만들었다가 스크린샷
    ...
    붙고 라벨 겹침 없음)
  - **B-2 보완 ✅ (2026-07-21, 사용자 지적)**: 실루엣·거리 자 **키 비례
    스케일** — 고정 실루엣은 "선에 맞추기 = 픽셀 크기 고정 = 거리가 키에
    비례"라 작은 키를 근거리(깊이 편향)로 유도, 마커 픽셀 밴드(절대 거리
    기준)와 충돌한다는 사용자 지적이 물리적으로 타당해 반영. 발 위치는
    바닥선이라 키와 무관 → **발 기준 고정(bottom 12%) + 높이만 프로필 키에
    비례**(기준 170cm=83%, 스케일 클램프 0.75~1.05). 기준물 칩도 실루엣
    가슴을 따라가도록 인라인 계산(기존 튜닝값 24%/28.5% 역산 상수 유지).
    `App.tsx`가 profile.heightCm을 CameraView에 전달(계약 변경 없음 —
    props만). 안내 문구 2곳도 "머리·발을 노란 선에"로 갱신.
    검증: tsc+build, E2E profile-flow 22/22(신규 2건: 150/172 비율 0.872
    정확·발 위치 고정 0.0px), 회귀 analyze 17/17·autoshoot 10/10(실백엔드),
    스크린샷 2장 육안 확인(172 vs 150 — 실루엣·머리 선·카드 박스가 비례
    이동, 발 선 고정)
  - **검증 자산화 (2026-07-21)**: B-1·B-2 육안 확인에 쓴 스크린샷 도구를
    `tests/screenshot-camera.mjs`로 보존 (13-4 — 사용법 주석 포함, 키·모드
    인자화. B-3·B-4 육안 확인에도 사용 예정). 실행 검증 완료(정밀·키 160)
  - **B-3 (③) ✅ 완료 (2026-07-21)**: 층위 1 안내 그림화 — 신규
    `GuideIllustration.tsx`(인라인 SVG 픽토그램 5종: 밀착✓/헐렁✗ 티셔츠,
    가슴 기준물(카드=가로 직사각형·마커=ArUco 정사각형 — 모드 분기 B-1 연동),
    폰↔사람 "약 2m" 화살표, 배꼽~가슴 높이 점선, 프레임 정중앙+가장자리
    붉은 왜곡 구역). 외부 이미지 0(오프라인·번들 영향 없음), 색은 기존
    오버레이 팔레트(노랑 거리 자·초록/빨강) 재사용. GUIDE_ITEMS는
    icon(이모지)→kind로 교체, 문구는 전부 유지 — 판정 로직·자동 촬영·TTS
    무변경(`CameraView.tsx` 배선 + `index.css` 스타일만).
    `screenshot-camera.mjs`에 guide 인자 추가(안내 열린 채 캡처).
    검증: tsc+build, E2E profile-flow 23/23(신규 1건: 픽토그램 SVG 5개),
    회귀 analyze 17/17·autoshoot 10/10(실백엔드), 스크린샷 2장(간편·정밀)
    육안 확인 — 5종 전부 의도대로 렌더, 카드/마커 분기 정상
  - **B-4 (⑥) ✅ 완료 (2026-07-21) → 백로그 ③~⑥ 전체 완료**: 촬영 후 실패
    시각화 — `MeasureResult.tsx` ok:false 분기 확장: **서버가 본 사진을
    그대로 표시**(CapturedImage 대표 프레임) + 기준물이 검출된 경우
    `reference.cornersPx`로 **초록 검출 테두리+라벨 오버레이**(SVG viewBox=
    원본 픽셀 좌표 — 히트맵 5-3d와 같은 스케일 원리) + **실패 단계별 원인
    체크리스트**(미검출: 카드=세로 방향·대비·가림 / 마커=평평·프레임·밝기
    — 1차 실기기 진단 그대로 / 신체 추출 실패: 기준물은 정상 안내 + 전신
    프레임·밝기). 백엔드 프로덕션 코드 무변경 — 기존 응답 데이터만 활용.
    가짜 백엔드에 시나리오 2종 추가(FITME_E2E_NOBODY=추출 실패 강제,
    FITME_E2E_NODETECT=/analyze 검출만 미스 — /check-photo는 실검출이라
    자동 촬영 유지). 검증: tsc+build, e2e-analyze **27/27**(신규 10건:
    사진 표시·polygon 유/무·단계별 체크리스트), 회귀 profile-flow 23/23·
    clothing-url 11/11·fit 10/10·synthesize 12/12·clothing-spec 13/13,
    dist 번들 API 키 grep 청정, 스크린샷 2장(NOBODY/NODETECT) 육안 확인
- **5-4 실기기 재검증 미수행 — 통과 가정 처리 (2026-07-21 사용자 결정, 정직
  기록)**: 환경(백엔드+프리뷰+터널)까지 기동했으나 사용자 결정으로 검증을
  수행하지 않고 통과를 가정, **배포 준비 단계로 진행**. 이전 보류(2026-07-17·
  18 Phase 2 수동 검증)와 동일한 방식 — **Gate는 통과 처리하지 않고 "미완"으로
  기록**(규칙 3의 예외는 사용자 명시 지시에 근거). 절차·통과 기준(A파트 정밀
  3회+간편 1회, 위 체크리스트)은 그대로 유효 — 추후 수행 시 qa Gate 리포트
  작성. **미검증 리스크**: B안 개선(거리 밴드 40~55px·TTS·거리 자·B-1~B-4)의
  실기기 실효성이 확인되지 않음 — 1차 세션 A파트 실패(척도 불일치 r=1.09~1.11)
  가 재발할 수 있는 상태. 배포 후 실사용에서 문제 발생 시 이 검증부터 수행할 것

### ⚠️ Phase 5 Gate 상태 (2026-07-21, qa — 정직 기록)

- **자동 검증: 통과** — 5-4 자동 파트 (pytest 168/168, E2E 전 스위트,
  FitResult.imageUrl 배선, 히트맵 픽셀 검증)
- **수동 검증: 부분** — B파트(합성 육안·히트맵 체감) 1차 세션 통과 ✓ /
  A파트(측정 일관성)는 **미수행 — 통과 가정(사용자 결정)**
- **통합 검증: 1차 세션 통과** — 실기기 촬영→측정→의류→핏→합성 완주 ✓
- **라이선스 항목: ❌ 미충족** — VTON 모델이 여전히 비상업(cuuupid/idm-vton,
  CC BY-NC-SA 4.0). **공개 배포 전 FASHN API 교체 필수** (12장)
- **판정: 공개 배포 조건 미충족** — 라이선스 교체 없이 대중 공개 불가

- **배포 방침 확정 (2026-07-21 사용자 결정)**: 1차 배포는 **주변인 대상 무료
  베타** — Replicate idm-vton 유지(수익 0·광고 0, 비상업 해석 그레이존 인지
  후 사용자 결정). **수익화 시점에 FASHN 교체가 필수 선행 조건** (CLAUDE.md
  12장에 동일 기록). 선택지 3종(FASHN 즉시 교체/합성 제외 배포/현 모델 유지)
  중 사용자가 3안 선택
- **Phase 6 (배포) 🔄 진행 중 (2026-07-21 시작)** — 5개 Step 분해안 사용자
  승인: 6-1 남용 방어(백엔드) / 6-2 고지 문구+베타 코드 입력(프론트) /
  6-3 배포 패키징(FastAPI가 dist 정적 서빙) / 6-4 호스팅+공개 URL /
  6-5 배포 URL 실기기 통합 검증. **호스팅 = 클라우드 컨테이너 확정
  (사용자 결정 — Railway 등, 월 $5 수준)**
  - **6-1 ✅ 완료 (2026-07-22)**: 남용 방어 — `services/access_guard.py` 신규.
    ① 베타 코드(X-Beta-Code 헤더, 주변인 공유) 틀리면 403 ② IP당 일일 상한
    (기본 10회 ≈ $1.5) ③ 서버 전체 일일 상한(기본 60회 ≈ $9 — IP 위조 우회의
    최종 방어선) 초과 시 429. 카운터는 SQLite(usage.sqlite, gitignore)라
    재배포에도 유지. **FITME_BETA_CODE 환경 변수 설정 시에만 활성** — 로컬
    개발·pytest·E2E는 완전 무변경(하위 호환). AI 비용 라우트(/analyze·
    /synthesize)에만 dependency 배선, /health·/check-photo·/clothing·/fit은
    비보호(비용 0). 거부된 요청은 카운트 안 함(틀린 코드 연타가 상한 잠식
...
- **UX 테마 전환 ✅ 완료 (2026-07-22, 사용자 요청 — 배포 전 진행)**: 다크 →
  **그레이지 베이지 라이트 테마** (팔레트 후보 3종 목업 제시 → 사용자 2안 선택).
  ...
  - **잔여**: 실기기 육안 확인은 6-5 배포 검증 세션에 포함(별도 세션 불필요 —
    ...
    사진 위 가독성은 기존 채도가 유리
  - **실기기 검증 세션 시도 (2026-07-22) — 환경 기동·검증 결과 미보고 상태로
    세션 보존**: 사용자 요청으로 그레이지 실기기 확인 환경을 구성 —
    **6-3a 배포 구성 그대로**(preview 4173 대신 FastAPI 8000 단일 유닛이
    dist+API 서빙, 터널 1개) cloudflared 터널로 노출, 공개 URL 경유
    프론트·/api/health 정상 확인(= 6-3a 정적 서빙의 터널 경유 실검증 겸함.
    프로덕션 번들이라 StrictMode 이중 전송 없음, 베타 게이트는 로컬 env
    미설정이라 미표시 = fail-open 정상). **사용자의 실기기 확인 결과는
    미보고** — 세션 보존 요청으로 터널·백엔드 종료(API 키 보호 규칙).
    재개 방법: ① server에서 uvicorn main:app --port 8000 ②
    cloudflared tunnel --url http://localhost:8000 → 출력된 https URL을
    폰에서 접속 (dist는 그레이지 반영 빌드 완료 상태). 확인 포인트는
    UX 육안(각 화면 베이지톤 + 카메라 화면은 기존 그대로 + 폰 상태바색) 및
    선택적으로 A파트 측정 검증(정밀 3회+간편 1회, ≈$0.6)
