...
- **Phase 2: 🔄 진행 중 — Step 2-3 완료** (2026-07-15, 실물 마커 검출 확인 + 테스트 16/16)
- **다음 시작 지점: Step 2-4 — 호모그래피 변환 (원근 제거 + px→mm 척도 산출)**
  - 만들 것: `server/services/measure.py` — 기준물 4꼭짓점(카드/ArUco 공통
    인터페이스) → `cv2.getPerspectiveTransform`으로 원근 제거 + 척도 계산
  - 검증: 크기를 아는 물체의 합성 이미지로 척도 오차 검증 (실사진은 참고용)
  - ⚠️ 시작 전 조건부 숙제 확인: 아래 "주의사항" 원본 해상도 항목
...
- ~~2-1 FastAPI~~ ✅ → ~~2-2 카드 검출~~ ✅ → ~~2-3 ArUco~~ ✅ → **2-4 호모그래피(다음)**
