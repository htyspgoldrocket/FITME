...
- **다음 시작 지점: Phase 4 교정 (사용자와 교정안 협의 중)** — 후보:
  (a) 상품명·종류 키워드로 핏 유형(오버핏 등)·신축 허리(트랙팬츠·조거·밴딩)
  감지 → 이상 여유 상향 / 허리 필터 완화 (b) actual-size data:null을
  "실측 미제공 상품" 안내로 구분 (c) 교정 후 같은 3종 + 추가 1종(자켓 대체)
  재검증
