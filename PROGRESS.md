...
- **Phase 2: 🔄 진행 중 — Step 2-2 완료** (2026-07-14, 사용자 검출 확인 + 자동 테스트 10/10)
- **다음 시작 지점: Step 2-3 — ArUco 마커 검출 (정밀 모드)**
  - 만들 것: `reference_detect.py`에 ArUco 검출 추가(`cv2.aruco`, 4.9 최신 API),
    마커 PDF 생성 스크립트 `server/scripts/generate_marker.py` → `server/data/aruco_marker.pdf`
  - 필요물: 생성한 마커를 사용자가 100% 배율로 출력해 촬영한 사진
    (`person01_aruco.jpg`) — 마커 PDF를 만들어 준 뒤 요청할 것
