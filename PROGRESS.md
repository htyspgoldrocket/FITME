...
- **다음 시작 지점: Step 2-7 — 다중 프레임 중앙값 + 해부학 비율 검증 + 신뢰도 (전략 3)**
  ...
  - 이후 2-7b(키 캘리브레이션+몸무게 보정+정의 계수+밀착 의류 재촬영 재검증) → 2-8
  - **사용자 준비물: 밀착 의류(통 좁은 하의 필수)로 재촬영한 마커 전신 사진**
    (2-7b 재검증에 필요 — 기존 반바지 사진은 엣지 케이스 자료로 보존)
...
- ~~2-1 FastAPI~~ ✅ → ~~2-2 카드~~ ✅ → ~~2-3 ArUco~~ ✅ → ~~2-4 호모그래피~~ ✅
  → ~~2-5 Claude Vision~~ ✅ → ~~2-6 cm 산출~~ ⚠️(개발 완료, 실측 0/7 — 재검증은 2-7b)
  → **2-7 다중 프레임·신뢰도(다음)** → 2-7b 정확도 보강(키·몸무게·정의 계수
  + 밀착 의류 재촬영 재검증) → 2-8 프론트 연결(+입력 UI, 밀착 의류 문구)
