...
- **다음 시작 지점: Phase 3 (의류 데이터 파이프라인)** — Step 분해안 사용자
  승인 후 3-1(의류 URL 입력 UI)부터. ⚠️ Phase 4 수동 검증(아는 옷 대조)은
  Phase 2 편향 일정성 가정의 실질 검증이므로, 늦어도 그 전에 Phase 2 수동
  검증(반복 일관성)을 완료할 것
- (이하 절차 기록은 추후 수동 검증 재개용으로 보존)
  ...
  - **Gate 수동 검증 절차 (세션 무관 재개용 — 결과 대기 중)**:
    1. 환경: `server/venv/Scripts/uvicorn.exe main:app --port 8000` +
       `npm run dev` + cloudflared 터널(5173) — 터널 1개로 백엔드 커버
       (/api 프록시). ⚠️ 터널은 검증 시간에만 (API 키 보호)
